...
import { pdfListaDelCorte } from "@/lib/servicios/tiktok-despacho";
...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const pdf = await pdfListaDelCorte(clienteAdmin(), cuenta.id, corteId);
    return new NextResponse(new Uint8Array(pdf), {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `inline; filename="corte-${corteId}-lista.pdf"`,
      },
    });
  } catch (err) {
