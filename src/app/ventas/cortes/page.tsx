...
export default async function Cortes({ searchParams }: { searchParams: Promise<{ mes?: string }> }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="titulo-pagina">Cortes y ganancia</h1>
        <p className="mt-0.5 text-sm" style={{ color: "var(--ink-2)" }}>
          Lo que se ganó de verdad en el mes: el depósito real de Mercado Pago, menos devoluciones, menos el costo del
          producto, menos publicidad y gastos de Full. Las órdenes canceladas quedan fuera. Todo al centavo, orden por
          orden.
        </p>
      </div>
      ...
      <div className="tarjeta flex flex-wrap items-center gap-3 p-3 text-sm">
        <Link href={`/ventas/cortes?mes=${periodoAnterior(periodo)}`} className="rounded-full border px-3 py-1 text-xs font-medium" style={{ borderColor: "var(--borde)" }}>
          ← {nombreDelPeriodo(periodoAnterior(periodo))}
        </Link>
        <span className="text-base font-semibold">{nombreDelPeriodo(periodo)}</span>
        {periodo < hoy ? (
          <Link href={`/ventas/cortes?mes=${periodoSiguiente(periodo)}`} className="rounded-full border px-3 py-1 text-xs font-medium" style={{ borderColor: "var(--borde)" }}>
      ...
      <AccionesCorte periodo={periodo} pendientes={e.revision.pendientes} cargosLeidos={e.cargosLeidos} corteId={corteDelMes?.id ?? null} />
      {corteDelMes ? (
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="tarjeta overflow-hidden">
          ...
          <GastosDelMes gastos={e.gastosManuales} desde={e.desde} hasta={e.hasta} />
        </section>
              ...
              {cortes.map((c) => (
                <tr key={c.id}>
                  <td className="font-medium">
                    <Link href={`/ventas/cortes?mes=${c.periodo}`} style={{ color: "var(--acento)" }}>
                  ...
                  <td className="num">
                    <a href={`/api/ventas/cortes/${c.id}/pdf`} target="_blank" rel="noreferrer" style={{ color: "var(--acento)" }}>
                      PDF
                    </a>
                  </td>
                </tr>
