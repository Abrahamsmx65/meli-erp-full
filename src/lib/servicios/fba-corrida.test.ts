...
// El objetivo real por talla es 44 días (30 + 14 de recepción en FBA).
const STOCK_AL_DIA = 44; // 1/día × 44 días = surtida justo al objetivo
...
describe("sugerirEnvioFba con la regla de la corrida despareja", () => {
  it("hermanas al día: viaja la mitad de las cajas de la talla agotada", () => {
    // La 22 vende 4/día y está en cero: pide 4 × 44 = 176 pares → 8 cajas.
    ...
    expect(s.faltantePares).toBe(176);
    // La mitad de las 8 cajas que completarían sus 44 días.
  ...
  it("corrida dispareja: solo se cubren los próximos 7 días", () => {
    ...
    // stock, más de 1.3× su objetivo. Solo viajan los 7 días de la 22.
    const filas = [
      renglon("22", 120, 0),
  ...
  it("si la mayoría de las tallas tiene faltante, no se recorta nada", () => {
    ...
    // 3 × 176 = 528 pares → 22 cajas completas, sin recorte.
    expect(s.cajas).toBe(22);
  });
