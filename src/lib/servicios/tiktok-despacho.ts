 ...
 * guías, con el mismo orden y los mismos "#n". Se dibuja en
 * `tiktok/empaque-pdf.ts` (sin base, para poder verla antes): cada modelo
 * en su propia hoja y arriba de cada hoja lo que hay que surtir para ella.
 */
export async function pdfListaDelCorte(admin: any, accountId: string, corteId: number): Promise<Uint8Array> {
  const corte = await cargarCorte(admin, accountId, corteId);
  return pdfListaDeEmpaque(corte);
}
