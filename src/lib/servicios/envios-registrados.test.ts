 ...
 */
function dbEnvios(filas: Record<string, unknown>[]) {
  const updates: { cambios: Record<string, unknown>; filtros: [string, string, unknown][] }[] = [];
  const db = {
    from(tabla: string) {
      if (tabla !== "envios_full") throw new Error(`tabla inesperada: ${tabla}`);
      const estado = {
      ...
      const q = {
        ...
        then(res: (r: unknown) => unknown) {
          const elegidas = filas.filter((f) => estado.filtros.every((fn) => fn(f)));
          if (estado.update) {
