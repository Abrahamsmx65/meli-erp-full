...
describe("listarPedidos con más de 1,000 líneas", () => {
  ...
  it("cuenta las cajas de más de 1,000 contenedores", async () => {
    const contenedores = Array.from({ length: 1050 }, (_, i) => ({
      ...
      contenedor_lineas: [{ cajas: 1, pedido_linea_id: "l1" }],
    }));
...
describe("listarContenedores con más de 1,000 líneas amarradas", () => {
  it("amarra cada contenedor con su pedido por tandas de ids", async () => {
    ...
    // jalón habría pasado de 1,000 ids (URL kilométrica y tope de filas).
    ...
    const contenedores = [0, 1, 2].map((c) => ({
      ...
      estado: "en_transito",
      notas: null,
      contenedor_lineas: lineas.slice(c * 400, (c + 1) * 400).map((l) => ({ cajas: 1, pedido_linea_id: l.id })),
    }));
