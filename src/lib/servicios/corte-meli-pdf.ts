...
export async function pdfDelCorte(e: EstadoResultados, opts?: { preliminar?: boolean; negocio?: string }): Promise<Uint8Array> {
  ...
  const cascada: Renglon[] = [
    ...
    { etiqueta: "Neto depositado por Mercado Pago", monto: puente.netoDepositado, tipo: "total", nota: e.ventaSinDeposito ? `${pesosPdf(e.ventaSinDeposito)} de venta sin depósito leído: no incluida` : e.netoEstimado > 0 ? `${pesosPdf(e.netoEstimado)} estimado (corte guardado antes)` : "depósito real de todas las órdenes" },
