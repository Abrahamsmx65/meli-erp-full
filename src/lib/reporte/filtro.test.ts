...
describe("paridad pantalla ↔ Excel en Bodega", () => {
  ...
  it("con textos precalculados filtra idéntico y USA el precalculado (no lo reconstruye)", () => {
    const textos = renglones.map(textoBusquedaBodega);
    // Idéntico con y sin precálculo, para cualquier filtro.
    for (const filtro of [{}, { q: "gt110 navy" }, { q: "IN10003", almacen: "Industher" }, { conCeros: true }]) {
      expect(filtrarBodega(renglones, filtro, textos)).toEqual(filtrarBodega(renglones, filtro));
    }
    // Y de verdad busca sobre el arreglo dado: con textos falsos, cambiar la
    // q encuentra por el texto inyectado, no por los campos del renglón —
    // prueba de que teclear no reconstruye la representación de las filas.
    const falsos = renglones.map((_, i) => (i === 0 ? "CLAVE-INYECTADA" : "nada"));
    const porFalso = filtrarBodega(renglones, { q: "clave-inyectada", conCeros: true }, falsos);
    expect(porFalso).toHaveLength(1);
    expect(porFalso[0].sku).toBe(renglones[0].sku);
    expect(filtrarBodega(renglones, { q: renglones[0].modelo, conCeros: true }, falsos)).toHaveLength(0);
  });

  it("busca por SKU, modelo, color, talla y pedido", () => {
    const conTodo = { conCeros: true };
    expect(filtrarBodega(renglones, { ...conTodo, q: "GT143-CREAM-24-MX" })).toHaveLength(1);
    expect(filtrarBodega(renglones, { ...conTodo, q: "gt135" })).toHaveLength(1);
    expect(filtrarBodega(renglones, { ...conTodo, q: "dk brown" })).toHaveLength(1);
    expect(filtrarBodega(renglones, { ...conTodo, q: "navy 27" })).toHaveLength(1);
    expect(filtrarBodega(renglones, { ...conTodo, q: "in10001" })).toHaveLength(2);
  });
