-- 0080 — El corte cuenta lo que aún no tiene desglose y lo que ya toca revisar
--
-- 1. `sin_desglose_total` / `sin_desglose_neto`: venta y depósito de las
--    órdenes vivas cuyo desglose de Mercado Pago aún no se lee. Con esto el
--    corte usa SIEMPRE el desglose por orden (antes, UNA orden sin desglose
--    lo tiraba al cálculo residual, donde la comisión y el envío
--    reconstruidos de las reventas aparecían como un descuadre de $262 mil)
--    y lo que falta entra como «sin desglose» exacto: total − depósito.
-- 2. `pendientes_vencidas`: órdenes a las que YA les toca su revisión (10 o
--    40 días); las demás aún no vencen y no son un faltante del corte.
-- Mismas funciones que 0076 con esas tres columnas al final.
...
returns table (
  ...
  sin_desglose_total numeric, sin_desglose_neto numeric, pendientes_vencidas bigint
)
language sql stable security definer set search_path = public as $$
  ...
  select o.fecha,
    ...
    coalesce(sum(o.total) filter (where o.estado is distinct from 'cancelled' and o.cargos_leidos_en is null), 0),
    coalesce(sum(o.neto_original) filter (where o.estado is distinct from 'cancelled' and o.cargos_leidos_en is null), 0),
    count(*) filter (where (o.revisiones = 0 and o.fecha <= (now() at time zone 'America/Mexico_City')::date - 10) or (o.revisiones = 1 and o.fecha <= (now() at time zone 'America/Mexico_City')::date - 40))::bigint
