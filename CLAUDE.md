...
| Packing lists desde la carpeta de Drive de la fábrica (cron diario 13:00Z + botón; entran como contenedor `borrador` que el dueño confirma; solo calzado: lo que no amarra con un pedido se omite; un contenedor confirmado no se toca; bitácora en `drive_packing_lists`, migración 0077). Llaves: `GOOGLE_DRIVE_API_KEY`, `DRIVE_PACKING_FOLDER_ID` | `src/lib/servicios/drive.ts` + `drive-packing.ts` + `/api/cron/packing-lists` + `/api/contenedores/drive` + `components/packing-drive.tsx` |
