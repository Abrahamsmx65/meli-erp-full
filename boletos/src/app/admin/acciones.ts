...
import { guardarEvento, type DatosEvento } from "@/lib/eventos";
...
export async function accionGuardarEvento(id: string | null, form: FormData): Promise<Resultado> {
  ...
  const datos: DatosEvento = {
    ...
    lugar: String(form.get("lugar") ?? "").trim() || null,
  ...
  if (!(datos.capacidad > 0)) return { error: "La capacidad debe ser mayor a cero." };
