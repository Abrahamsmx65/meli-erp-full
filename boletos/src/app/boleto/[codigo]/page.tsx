...
export default async function PaginaBoleto({ params }: { params: Promise<{ codigo: string }> }) {
  ...
  if (!datos) notFound();
  const { boleto, pedido, evento } = datos;
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-md px-4 pb-16">
        <article className="tarjeta boleto-borde overflow-hidden">
          <div className="p-6 text-center" style={{ background: "var(--tinta)", color: "#fff" }}>
            <div className="text-xs uppercase tracking-widest opacity-70">Boleto {formatearFolio(boleto.folio)}</div>
            <h1 className="mt-1 text-2xl font-extrabold">{evento.nombre}</h1>
            <p className="mt-1 text-sm opacity-80">{fechaLarga(evento.fecha)}</p>
            {evento.lugar && <p className="text-sm opacity-80">{evento.lugar}</p>}
          </div>
          ...
          <div className="grid place-items-center p-6">
            ...
            <div className="mono mt-3 text-xs tracking-wider" style={{ color: "var(--tinta-suave)" }}>
              {boleto.codigo}
            </div>
          </div>
        ...
        <p className="no-imprimir mt-4 text-center text-xs" style={{ color: "var(--tinta-suave)" }}>
          Muestra este QR en la entrada. Puedes imprimirlo o presentarlo desde el celular.
        </p>
      </main>
    </>
  );
